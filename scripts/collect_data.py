"""
collect_data.py — PokéWorld v2.

Runs the frozen PPO policy (or random actions) to collect diverse trajectories.
Extracts downsampled (72, 80, 3) frames, actions, and relevant RAM states,
then dumps them in compressed .npz chunks.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Collect high-resolution trajectories for RSSM training")
    ...
    p.add_argument("--total-steps", type=int, default=200000,
                   help="Total number of steps to collect (default: 200k)")
...
def main() -> None:
    ...
    while total_steps_collected < args.total_steps:
        ...
        while step_in_ep < ep_steps_limit and total_steps_collected < args.total_steps:
            ...
            # Use native resolution (160x144x3)
            downsampled_frame = raw_frame.copy()
            
            ram_state = extract_game_state(pyboy)

            # 2. Decide action using PPO policy (which internally downsamples to 40x36) or randomly
            if rand_flag or policy is None:
                action = random.randint(0, 5) # 6 actions: DOWN, LEFT, RIGHT, UP, A, B
            else:
