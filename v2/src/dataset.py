...
class PokemonDataset(Dataset):
    ...
    Supports returning individual frames or trajectory sequences.
    """
    def __init__(self, data_dir: str | Path, seq_len: int = 15, transform = None, max_files: int = None):
        ...
        self.obs = np.concatenate(all_obs, axis=0) # (N, 72, 80, 3)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.seq_len == 1:
            # Single frame mode (reconstructed normalized frame)
            img = self.obs[start_idx] # (72, 80, 3)
            ...
            # Trajectory sequence mode (for RSSM sequence training)
